...
public class QsharpSdkConnector implements SdkConnector {
    ...
    public QsharpSdkConnector(
        ...
        transpileAPIEndpoint = URI.create(String.format("http://%s:%d/qsharp-service/api/%s/transpile", hostname, port, version));
        executeAPIEndpoint = URI.create(String.format("http://%s:%d/qsharp-service/api/%s/execute", hostname, port, version));
    }
    ...
    private void executeQuantumCircuit(QsharpRequest request, ExecutionResult executionResult, ExecutionResultRepository resultRepository) {
        ...
        try {
            ...
            executionResult.setStatusCode("Pending for execution on Qsharp Service ...");
    ...
    public List<String> supportedSdks() {
        return Arrays.asList(Constants.QSHARP);
    }
    ...
    public List<String> supportedProviders() {
        return Arrays.asList(Constants.MICROSOFT);
    }
